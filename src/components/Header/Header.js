...
export default function Header() {
  ...
  useEffect(() => {
    getNotification(context.user.id).then((doc) => {
      if (doc) {
        setIsSeen(false);
        setNotifi(doc.noti);
      }
      ...
      <div className="nav-content">
        ...
        <div className="right-nav">
          <div className="content-right-nav">
            ...
            <div onClick={openNoti} className="content-item wrapper-noti">
              {notifi && <div className={`${isSeen ? "" : "noti"}`} />}
              <div className="zx">
                <svg
              ...
              <div className="t-modal">
                ...
                {notifi &&
                  notifi.map((item, index) => (
                    <div key={index} className="modal-noti">
                      <div className="byUser">
                        <a href="/">
                          <img
                            src={`${context.actions.getUrlAvatar(item.byUser)}`}
                            alt="not found"
                          />
                        </a>
                      </div>
                      <div className="content">
                        <a href="/">
                          {context.actions.getUserNameById(item.byUser)}
                        </a>{" "}
                        đã thích ảnh của bạn
                      </div>
                      {/* <div className="post">
                        <a href="/">
                          {console.log("doc", item.postId)}
                          <img
                            src={getUrlPostLikedOrComment(item.postId).then(
                              doc => doc.imageUrls[0]
                            )}
                            alt="not found"
                          />
